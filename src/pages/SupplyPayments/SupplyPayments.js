...
export default function SupplyPayments() {
    ...
    const [table_head, setTableHead] = useState([
        ...
        {id: 'SuppliesId', label: 'SuppliesId', alignRight: false},
    ...
    useEffect(() => {
        axios.get("http://127.0.0.1:9000/api/suppliespayment").then((response) => {
    ...
    const handleTextInput = () => {
        ...
        axios.post("http://127.0.0.1:9000/api/suppliespayment", [form_data]).then((response) => {
    ...
    const deleteItem = (_id) => {
        if (Array.isArray(_id)) {
            _id.map((item) => {
                axios.post("http://127.0.0.1:9000/api/suppliespayment/delete", [{"_id": item}]).then((response) => {
                    console.log(response.data.info)
                    setListUpdate(!listUpdate);
                })
            })
            ...
            axios.post("http://127.0.0.1:9000/api/suppliespayment/delete", [{_id}]).then((response) => {
                console.log(response.data.info)
                setListUpdate(!listUpdate);
            })
        }
    ...
    const handleEditTextInput = () => {
        ...
        form_data["Date"] = new Date().toLocaleDateString();
        form_data["Time"] = new Date().toLocaleTimeString();
        axios.post("http://127.0.0.1:9000/api/suppliespayment/update", form_data).then((response) => {
            console.log(response.data.info);
            setListUpdate(!listUpdate);
        });
        console.log(form_data);
    ...
    const editItem = (_id) => {
        ...
        setEditId(_id);
        axios.get(`http://127.0.0.1:9000/api/suppliespayment?_id=${_id}`).then((response) => {
            console.log(response.data.info);
