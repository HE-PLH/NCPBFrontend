...
export default function InvoicesView() {
    ...
    const [table_head, setTableHead] = useState([
        ...
        {id: 'LPONumber', label: 'LPONumber', alignRight: false},
        {id: 'DeliveryNumber', label: 'DeliveryNumber', alignRight: false},
        {id: 'ClientName', label: 'ClientName', alignRight: false},
        {id: 'items', label: 'items', alignRight: false},
        {id: 'CashTotal', label: 'CashTotal', alignRight: false},
        {id: 'ClientPhoneNumber', label: 'ClientPhoneNumber', alignRight: false},
        {id: 'ClientBox', label: 'ClientBox', alignRight: false},
        {id: 'ClientLocation', label: 'ClientLocation', alignRight: false},
        {id: 'ClientTargetDepartment', label: 'ClientTargetDepartment', alignRight: false},
        {id: 'CashVAT', label: 'CashVAT', alignRight: false},
        {id: 'CashVATPerc', label: 'CashVATPerc', alignRight: false},
        {id: 'TaxInclusive', label: 'TaxInclusive', alignRight: false},
        {id: 'CashDiscount', label: 'CashDiscount', alignRight: false},
        {id: 'CashSubtotal', label: 'CashSubtotal', alignRight: false},
    ...
    useEffect(() => {
        axios.get("http://127.0.0.1:9000/api/invoice").then((response) => {
    ...
    function getItemStringList(items) {
        let temp = "";
        let t_items = items.sort((a, b) => a.Id - b.Id);
    ...
    const deleteItem = (_id) => {
        if (Array.isArray(_id)) {
            _id.map((item) => {
                axios.post("http://127.0.0.1:9000/api/invoice/delete", [{"_id": item}]).then((response) => {
                    console.log(response.data.info)
                    setListUpdate(!listUpdate);
                })
            })
            ...
            axios.post("http://127.0.0.1:9000/api/invoice/delete", [{_id}]).then((response) => {
                console.log(response.data.info)
                setListUpdate(!listUpdate);
            })
        }
    ...
    const handleEditTextInput = () => {
        ...
        form_data["items"] = getItemStringList(form_data["items"]);
        form_data["Date"] = new Date().toLocaleDateString();
        form_data["Time"] = new Date().toLocaleTimeString();
        axios.post("http://127.0.0.1:9000/api/invoice/update", form_data).then((response) => {
            console.log(response.data.info);
            setListUpdate(!listUpdate);
        });
        console.log(form_data);
    ...
    const editItem = (_id) => {
        ...
        setEditId(_id);
        axios.get(`http://127.0.0.1:9000/api/invoice?_id=${_id}`).then((response) => {
            console.log(response.data.info);
