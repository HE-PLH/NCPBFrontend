...
export default function ProjectSites() {
    ...
    useEffect(() => {
        axios.get("http://127.0.0.1:9000/api/projectsites").then((response) => {
    ...
    const handleTextInput = () => {
        ...
        axios.post("http://127.0.0.1:9000/api/projectsites", [form_data]).then((response) => {
    ...
    const deleteItem = (_id) => {
        if (Array.isArray(_id)) {
            _id.map((item) => {
                axios.post("http://127.0.0.1:9000/api/projectsites/delete", [{"_id": item}]).then((response) => {
                    alert(response.data.info)
                    setListUpdate(!listUpdate);
                })
            })
            ...
            axios.post("http://127.0.0.1:9000/api/projectsites/delete", [{_id}]).then((response) => {
                alert(response.data.info)
                setListUpdate(!listUpdate);
            })
        }
    ...
    const handleEditTextInput = () => {
        ...
        form_data["Date"] = new Date().toLocaleDateString();
        form_data["Time"] = new Date().toLocaleTimeString();
        axios.post("http://127.0.0.1:9000/api/projectsites/update", form_data).then((response) => {
            alert(response.data.info);
            setListUpdate(!listUpdate);
        });
        console.log(form_data);
    ...
    const editItem = (_id) => {
        ...
        setEditId(_id);
        axios.get(`http://127.0.0.1:9000/api/projectsites?_id=${_id}`).then((response) => {
