...
export default function InvoicePayment() {
    ...
    useEffect(() => {
        axios.get("http://127.0.0.1:9000/api/invoicepayment").then((response) => {
    ...
    const handleTextInput = () => {
        ...
        // form_data["Id"] = v4();
        axios.post("http://127.0.0.1:9000/api/invoicepayment", [form_data]).then((response) => {
    ...
    const deleteItem = (_id) => {
        if (Array.isArray(_id)) {
            _id.map((item) => {
                axios.post("http://127.0.0.1:9000/api/invoicepayment/delete", [{"_id": item}]).then((response) => {
                    console.log(response.data.info)
                    setListUpdate(!listUpdate);
                })
            })
            ...
            axios.post("http://127.0.0.1:9000/api/invoicepayment/delete", [{_id}]).then((response) => {
                console.log(response.data.info)
                setListUpdate(!listUpdate);
            })
        }
    ...
    const editItem = (_id) => {
        ...
        setEditId(_id);
        axios.get(`http://127.0.0.1:9000/api/invoicepayment?_id=${_id}`).then((response) => {
            console.log(response.data.info);
    ...
    return (
        <Page title="Invoice Payments | NCPB">
            {showAppDialog && !showEditDialog ? (
                <AlertDialog
                    title={`Add Invoice Paymment`}
