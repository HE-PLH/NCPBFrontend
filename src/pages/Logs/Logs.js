...
export default function Logs() {
    ...
    const [table_head, setTableHead] = useState([
        ...
        {id: 'userSn', label: 'userSn', alignRight: false},
        {id: 'deviceUserId', label: 'deviceUserId', alignRight: false},
    ...
    useEffect(() => {
        axios.get("http://127.0.0.1:9000/api/BiometricLogs").then((response) => {
    ...
    const handleTextInput = () => {
        ...
        form_data["Id"] = v4();
        axios.post("http://127.0.0.1:9000/api/Logs", [form_data]).then((response) => {
    ...
    const deleteItem = (_id) => {
        if (Array.isArray(_id)) {
            _id.map((item) => {
                axios.post("http://127.0.0.1:9000/api/Logs/delete", [{"_id": item}]).then((response) => {
                    alert(response.data.info)
                    setListUpdate(!listUpdate);
                })
            })
            ...
            axios.post("http://127.0.0.1:9000/api/Logs/delete", [{_id}]).then((response) => {
                alert(response.data.info)
                setListUpdate(!listUpdate);
            })
        }
    ...
    const handleEditTextInput = () => {
        ...
        form_data["Date"] = new Date().toLocaleDateString();
        form_data["Time"] = new Date().toLocaleTimeString();
        axios.post("http://127.0.0.1:9000/api/Logs/update", form_data).then((response) => {
            alert(response.data.info);
            setListUpdate(!listUpdate);
        });
        console.log(form_data);
    ...
    const editItem = (_id) => {
        ...
        setEditId(_id);
        axios.get(`http://127.0.0.1:9000/api/Logs?_id=${_id}`).then((response) => {
