...
export default function SingleSupplyView() {
    ...
    useEffect(() => {
        axios.get(`http://127.0.0.1:9000/api/individualsupplies?_id=${params.Id}`).then((response) => {
            setSupplyProject(response.data.info[0]);
            return response.data.info[0];
        }).then((res) => {
            axios.get(`http://127.0.0.1:9000/api/supplies?Id=${res.SupplyId}`).then((response) => {
                return (response.data.info[0]);
            }).then((res) => {
        ...
        <Page title="Supplies | NCPB">
            ...
            <Box ml={1} mr={1}>
                ...
                <Stack direction="row" alignItems="center" justifyContent="space-between" maxWidth="50%" mb={1} ml={3}>
                    ...
                    <Typography variant="subtitle1" gutterBottom>
                        Date: {supplyProject.Date} - Time: {supplyProject.Time}
                    </Typography>
