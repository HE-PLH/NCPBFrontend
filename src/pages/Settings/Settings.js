...
export default function Settings() {
    ...
    useEffect(() => {
        axios.get("http://127.0.0.1:9000/api/settings").then((response) => {
    ...
    const handleTextInput = () => {
        ...
        axios.post("http://127.0.0.1:9000/api/settings", [form_data]).then((response) => {
    ...
    const deleteItem = (_id) => {
        if (Array.isArray(_id)) {
            _id.map((item) => {
                axios.post("http://127.0.0.1:9000/api/settings/delete", [{"_id": item}]).then((response) => {
                    console.log(response.data.info)
                    setListUpdate(!listUpdate);
                })
            })
            ...
            axios.post("http://127.0.0.1:9000/api/settings/delete", [{_id}]).then((response) => {
                console.log(response.data.info)
                setListUpdate(!listUpdate);
            })
        }
    ...
    const handleEditTextInput = () => {
        ...
        form_data["Date"] = new Date().toLocaleDateString();
        form_data["Time"] = new Date().toLocaleTimeString();
        axios.post("http://127.0.0.1:9000/api/settings/update", form_data).then((response) => {
            console.log(response.data.info);
            setListUpdate(!listUpdate);
        });
        console.log(form_data);
    ...
    const editItem = (_id) => {
        ...
        setEditId(_id);
        axios.get(`http://127.0.0.1:9000/api/settings?_id=${_id}`).then((response) => {
            console.log(response.data.info);
        ...
        <Page title="Settings | NCPB">
            ...
            <Box ml={1} mr={1}>
                ...
                <MyTable list={list} tableHead={table_head} deleteFunction={deleteItem}
                                editFunction={editItem} viewFunction={viewItem}/>
            </Box>
        </Page>
    );
}
