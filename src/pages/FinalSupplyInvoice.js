...
export default function FinalSupplyInvoice() {
                ...
                switch (response.data.info.status) {
                    ...
                    default:
                        ...
                        for (let i = 0; i < invoice.items.length; i++) {
                            let temp = {};
                            temp["_id"] = invoice.items[i].Id;
                            temp["InvoiceNumber"] = response.data.info.invoice.Id;
                            temp["InvoiceStatus"] = "Invoiced";
                            up_.push(temp);
                        }
    ...
    const createNew = () => {
        axios.post("http://127.0.0.1:9000/api/invoice/proceed", invoice).then((response) => {
            setMyInvoice(response.data.info.invoice);
            return (response.data.info.invoice)
        }).then((res) => {
            invoice ? (function () {
                let up_ = [];
                for (let i = 0; i < invoice.items.length; i++) {
            ...
            axios.post("http://127.0.0.1:9000/api/invoice/update", {_id: _id, Repeated: "Repeated"}).then((response) => {
                    console.log(response.data.info);
                });
        });
                    ...
                    <Box className={"item-container"} sx={{zIndex: 3, width: "95%", position: "absolute"}}>
                        <Stack direction="row" className={"print-top-row"} alignItems="center"
                            ...
                            <Box className={"invoice-title"} sx={{display: "flex", flexDirection: "column"}}>
                                ...
                                <Typography className={"subtitle2"} variant="subtitle2" gutterBottom>
                                    Invoice#: KI-{myInvoice.Id}
                                </Typography>
                                <Typography className={"subtitle2"} variant="subtitle2" gutterBottom>
                                    Date: {Methods.formatDate(new Date(myInvoice.Date))}
                                </Typography>
                            </Box>
                        </Stack>
                        ...
                        <Box className={"other-comments-sct"} display="flex" justifyContent="space-between" mt={3}>
                            ...
                            <Stack className={"other-comments-stack-right"} direction="column" width="40%"
                                ...
                                <Stack className={"other-comments-stack-1"} mt={3} ml={3}>
                                    <Typography className={"subtitle2"} variant="subtitle2" gutterBottom>
                                        Delivery Number: {myInvoice.DeliveryNumber}
                                    </Typography>
                                </Stack>
                            </Stack>
                        </Box>
                                    ...
                                    {myInvoice.items ? myInvoice.items.map((row) => {
                                        return (
                                            <TableRow className={"print-tr"}>
                                                ...
                                                <TableCell className={"print-td"} align="right" component="td"
                                                           scope="row"
                                                           padding="none">{Methods.numberWithCommas(parseFloat(row.Total).toFixed(2))}</TableCell>
                                            </TableRow>
                                        )
                                    }) : null}
                                </TableBody>
                            ...
                            <Stack className={"other-comments-stack-right"} direction="column" width="40%"
                                ...
                                >
                                    ...
                                    <Box className={"other-comments-row-value"} minWidth="50%" display="flex"
                                        ...
                                        <Typography variant="subtitle1" gutterBottom>
                                            Ksh {myInvoice.CashSubtotal ? Methods.numberWithCommas(parseFloat(myInvoice.CashSubtotal).toFixed(2)) : 0} {myInvoice.TaxInclusive === "false" ? "(Exclusive)" : ""}
                                        </Typography>
                                    </Box>
                                </Box>
                                <Box className={"other-comments-row"} display="flex" direction="row"
                                ...
                                >
                                    ...
                                    <Box className={"other-comments-row-value"} minWidth="50%" display="flex"
                                         ...
                                         justifyContent="flex-end">
                                        <Typography variant="subtitle1" gutterBottom>
                                            Ksh {myInvoice.CashTotal ? Methods.numberWithCommas(parseFloat(myInvoice.CashTotal).toFixed(2)) : 0} {myInvoice.TaxInclusive === "true" ? "(Inclusive)" : ""}
                                        </Typography>
                                    </Box>
                                </Box>
                            </Stack>
