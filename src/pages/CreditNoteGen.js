...
import MyTable from "./no-string-credit-supply-tb";
import {printElement} from "../utils/utilities";

// ----------------------------------------------------------------------

export default function CreditNoteGen() {
    const {setCredit} = useContext(ShowAppDialogContext);
    ...
    useEffect(() => {
        ...
        items.map((Id) => {
            axios.get("http://127.0.0.1:9000/api/individualsupplies?_id="+Id).then((response) => {
                if (parseFloat(response.data.info[0].DeliveredQuantity)<parseFloat(response.data.info[0].Quantity)) {
    ...
    const sumOfItems=(lst)=>{
        ...
        if (lst.length>0) {
            for (let i = 0; i < lst.length; i++) {
                sum += parseFloat(parseFloat(lst[i].Quantity)-parseFloat(lst[i].DeliveredQuantity)) * parseFloat(lst[i].Price)
            }
        }
        return sum;
    ...
    const _print = ()=>{
        ...
        let lst = [ "_date", "_time", "_deliveryId", "_clientName", "_phoneNumber", "_location", "_targetDepartment", "_box", "_total", "_subTotal", "_VAT", "_discount", "_inclusive", "_LPONumber"];
        ...
        for (let i = 0; i < lst.length; i++) {
            switch (lst[i]) {
                ...
                case "_deliveryId":
                    form_data["CreditNoteId"] = document.querySelector(`.${lst[i]}`).value
                    break;
        ...
        /*axios.post("http://127.0.0.1:9000/api/delivery", [form_data]).then((response) => {
            setCredit(response.data.info);
        });*/
        // navigate(`deliveryTemplate/${site.Id}/${params.Id}/FinalSupplyCreditNote`);
    }
                            ...
                            <Stack>
                                ...
                                >
                                    ...
                                    <input
                                        ...
                                        id=""
                                        style={{marginLeft: "10px"}}
                                        placeholder={"Enter Credit Note Time"}
                                    />
                                </Box>
                            </Stack>
                        </Card>
