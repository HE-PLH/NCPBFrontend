...
import MyTable from "./PettyCash/tb";
import {printElement} from "../utils/utilities";
import SuppliesView from "./Supplies-View/Supplies";
import PettyCashExpenses from "./PettyCash/PettyCashExpenses";
import BiometricReports from "./Biometric Reports/BiometricReports";
...
export default function PettyCash() {
    ...
    const [petty_cash_head, setPettyCashHead] = useState([
        {id: 'Description', label: 'Description', alignRight: false},
        {id: 'Target', label: 'Target', alignRight: false},
        {id: 'Amount', label: 'Amount', alignRight: false},
        {id: 'Quantity', label: 'Quantity', alignRight: false},
    ...
    useEffect(() => {
        setFlag1(false);
        setChoice("Expenses");
        axios.get(`http://127.0.0.1:9000/api/pettycash`).then((response) => {
    ...
    const handleTextInput = () => {
        ...
        axios.post("http://127.0.0.1:9000/api/pettycash", [form_data]).then((response) => {
    ...
    const deleteItem = (_id) => {
        if (Array.isArray(_id)) {
            _id.map((item) => {
                axios.post("http://127.0.0.1:9000/api/pettycash/delete", [{"_id": item}]).then((response) => {
            ...
            axios.post("http://127.0.0.1:9000/api/pettycash/delete", [{_id}]).then((response) => {
                alert(response.data.info)
                setListUpdate(!listUpdate);
            })
        }
    ...
    const handleEditTextInput = () => {
        ...
        // form_data["Date"] = new Date().toLocaleDateString();
        form_data["Time"] = new Date().toLocaleTimeString();
        axios.post("http://127.0.0.1:9000/api/pettycash/update", form_data).then((response) => {
            alert(response.data.info);
            setListUpdate(!listUpdate);
        });
        console.log(form_data);
    ...
    const editItem = (_id) => {
        ...
        setEditId(_id);
        axios.get(`http://127.0.0.1:9000/api/pettycash?_id=${_id}`).then((response) => {
            console.log(response.data.info);
            ...
            <Box ml={1} mr={1} maxWidth="xl">
                <Box sx={{justifyContent: "center", display: "flex"}}>
                    ...
                    <Button
                        ...
                        onClick={(e) => handleThirdClick(e, "Petty Cash Summary")}
                    >
                        Petty Cash Summary
                    </Button>
                </Box>
                    ...
                    choice === "Expenses" ?
                        <Box>
                            ...
                            <Box ml={1} mr={1}>
                                ...
                                <MyTable list={list} tableHead={petty_cash_head} deleteFunction={deleteItem}
